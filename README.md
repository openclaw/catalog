...
The v3 catalog is staged at **https://catalog.openclaw.ai/models/v3/catalog.json**.
It adds typed inference metadata and route-specific pricing provenance. Decision-only
models belong in v3, not the chat-compatible v1/v2 model lists. An explicit zero
rate is distinct from an unknown rate; token usage is not itself a monetary price.
Model definitions and wire validation remain owned by the upstream publisher.
See [v3 rollout ordering](#v3-rollout-ordering) before enabling this feed.
...
and commits all three versions **only when any version's content actually changed** —
so the file history is a readable changelog of model additions and pricing updates.

One publisher invocation generates v1, v2, and v3 from the same source snapshot:

```sh
node --import tsx scripts/publish-model-catalog.mts --pricing \
  --out ../models/v1/catalog.next.json \
  --out-v2 ../models/v2/catalog.next.json \
  --out-v3 ../models/v3/catalog.next.json
```

Changes to generation timestamps or source commit metadata alone do not trigger
a commit. If any payload changes, all three outputs are committed together. A
missing, empty, multi-document, or syntactically malformed next output fails before
any committed file is replaced. Wire-schema validation stays with the upstream publisher.
...
`node scripts/build.mjs` stages the v1 catalog, its alias, the v2/v3 catalogs, and
...
The smoke check verifies all four URLs for version-specific byte equality,
...
retain the latest `models/v1/catalog.json`, `models/v2/catalog.json`, and
`models/v3/catalog.json` as a set,
...
1. Land [openclaw/openclaw#157492](https://github.com/openclaw/openclaw/pull/157492)
   and its prerequisites before activating this publication change. The workflow
   still checks out upstream main, never a development branch. Its generation
   preflight refuses an older publisher without `--out-v3`.
2. Land this hosting change only after that prerequisite. The initial v3 artifact
   is generated by the canonical publisher at source commit
   `c3bd84fb207ae2b585ec126c620e4790ca39f480`; it is not hand-authored test data.
   The existing v1/v2 artifacts are retained at bootstrap to avoid unrelated feed
   churn. Subsequent scheduled generations commit all versions as one source set.
3. Use the existing preview-first publication workflow to make the asset available
   and verify all four URLs. A local build or draft PR is not a deployment.
4. Only then may an operator select the v3 URL through the existing
   `models.catalogRefresh.url` setting on a v3-capable reader. No default URL or
   live configuration is changed here. Keep the unversioned alias on v1, and do
   not copy v1's older `minVersion` floor onto v3.

The generation and activation steps are separate: merging or inspecting an
upstream PR does not deploy this repository or authorize a live configuration
change. Roll back hosting through the forward-deployment procedure above, retaining
the latest generated version set.
